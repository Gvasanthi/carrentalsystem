...
$res_u="";
    ...
    $sql_u="Select c.carid ,c.carname ,c.carnumber ,ROUND(avg(f.rating),1) as rating,f.satisfy  from feedback f,cardetails c where f.carnumber=c.carnumber and c.carname not in (select carname from cardetails where carname='select') group by c.carid ,c.carname,c.carnumber";
...
<html>
<body>
		<h1 align="center" style="color: orange">Feedback Of Cars</h1>
...
while ($rep=mysqli_fetch_assoc($res_u))
{
	...
	echo "<td>".$rep['rating']."</td>";
	
	echo "<td>".$rep['satisfy']."</td>";
	echo "</tr>";
}
?>
